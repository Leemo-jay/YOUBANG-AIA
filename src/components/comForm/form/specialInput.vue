	...
	export default {
		...
		data() {
			return {
				...
				isShowPicker: false,
				arrList: [],
// 				1.身分证，18位，需进行身分证号码有效性校验；护照，最少7位；军人证、士兵证，必须为10-18位；回乡证，最少8位；出生证证件号码：只能填寫数字和字母（不能是中文和特殊字符），最大18位長度。户口本证件号码，校验同身分证号码。驾照、学生证、工作证、临时身分证、警官证、台胞证，港、澳、台通行证，这些证件号码先不做限制，按照数据库長度做个最大長度填寫限制即可。 其它、无证件，这种证件类型選项不需要在前端頁面显示
// 2.邮箱：只允许以“数字/字母/下划线/点/中划线+@+数字/字母/中划线/点”格式的邮箱填寫，不允许*、%、&、$等非法字符的填寫
// 3.手机号：11位数字
// 4.银行卡号：不得少于16位且都是数字
// 5.密碼：6至20位字符，可使用字母，数字和符号的组合，字母区分大小写。强制规则：密碼中必需至少要有字母和数字。
// 6.地址：不少于5个字符且需包含数字
